...
int main() {
    ...
    // std::vector<std::jthread> threads; // Use std::jthread for automatic join on destruction

    // for (int i = 0; i < 4; ++i)
    //     threads.emplace_back(raceCondition, i, shared_array.get());
    // for (int i = 0; i < 4; ++i)
    //     threads.emplace_back(safeThreadAccess, i, shared_array.get());

    // std::cout << "Race result: " << shared_array[0] << "\n";
    // std::cout << "Safe result: " << shared_array[1] << "\n";
